...
-- Данный запрос выводит 10-ку лучших продавцов, 
показывая их имя и фамилию, суммарную выручку с проданных товаров и 
количество проведенных сделок. Жанные отсортированны по 
убыванию выручки, избавившись от чисел после запятой, 
т.е. они округленны в меньшую сторону.
select 
...
-- Данный запрос выводит информацию о продавцах, чья 
средняя выручка за сделку меньше средней выручки за 
сделку по всем продавцам. Таблица отсортирована по 
выручке по возрастанию. Представлены имя и фамилия 
продавца, средняя выручка продавца за сделку с 
округлением до целого в меньшую сторону.
select 
concat(employees.first_name, ' ',employees.last_name) as seller,
...
-- Данный запрос содержит информацию о выручке по дням 
недели. Каждая запись содержит имя и фамилию продавца, 
название дня недели на английском языке и суммарную 
выручку продавца в определенный день недели, округленную 
до целого числа в меньшую сторону. Данные отсортированы по 
порядковому номеру дня недели и продавцу.
with tab as (select
...
-- Данный запрос показывает количество покупателей в 
разных возрастных группах: 16-25, 26-40 и 40+. Итоговая 
таблица отсортирована по возрастным группам.
...
-- Данный запрос показывает данные по количеству 
уникальных покупателей и выручке, которую они принесли. 
Данные сгруппированы по дате, которая представлена в 
числовом виде ГОД-МЕСЯЦ. Итоговая таблица отсортирована 
...
-- Данный запрос показывает данные о покупателях, первая 
покупка которых была в ходе проведения акций (акционные 
товары отпускали со стоимостью равной 0). Итоговая 
таблица отсортирована по id покупателя. Таблица содержит 
поля "имя и фамилия покупателя", "дата покупки", "имя и 
фамилия продавца".
with tab as
    (
